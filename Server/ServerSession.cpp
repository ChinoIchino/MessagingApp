...
void ServerSession::packetListener(){
    ...
    asio::async_read(
        this->sessionSocket,
        asio::buffer(this->header),
        [this, self](std::error_code ec, std::size_t length){
            if(!ec){
                uint16_t packetSize = (static_cast<uint16_t>(this->header[0]) << 8) | static_cast<uint16_t>(this->header[1]);
                std::cout << "Got the packetSize = " << (int)(packetSize) << std::endl;

                asio::async_read(
                    this->sessionSocket,
                    asio::buffer(this->readBuff, packetSize),
                    [this, self](std::error_code ec, std::size_t length){
                        if(!ec){
                            ...
                            this->server.removeConnection(shared_from_this());
                            return;
                        }
                    }
                );
...
void ServerSession::send(Packet* packet){
    ...
    // this->server.broadcast(self, packet);
    write(packet);
}
